...
class DatabaseHelper {
  ...
  Future<Database> _initDatabase() async {
    // Para web, retorna uma implementação mock ou lança erro
    if (kIsWeb) {
      throw UnsupportedError(
          'SQLite não é suportado na web. Use IndexedDB ou outra alternativa.');
    }
    ...
    return await openDatabase(
      ...
      onOpen: (db) {
        // Garante que as foreign keys estejam habilitadas
  ...
  Future<void> _insertDefaultData(Database db) async {
    try {
      ...
      final laboratoriosCount = Sqflite.firstIntValue(
              await db.rawQuery('SELECT COUNT(*) FROM laboratorios')) ??
          0;
      ...
      final distribuidorasCount = Sqflite.firstIntValue(
              await db.rawQuery('SELECT COUNT(*) FROM distribuidoras')) ??
          0;

      if (distribuidorasCount == 0) {
      ...
      final produtosCount = Sqflite.firstIntValue(
              await db.rawQuery('SELECT COUNT(*) FROM produtos_diversos')) ??
          0;
      ...
      print('Erro ao inserir dados padrão: $e');
      // Continua mesmo com erro para não bloquear a criação do banco
    }
  }
  ...
  // Método para verificar se o banco está inicializado
  Future<bool> isDatabaseInitialized() async {
    try {
      final db = await database;
      // Tenta fazer uma consulta simples para verificar se o banco está funcionando
      await db.rawQuery('SELECT 1');
      return true;
    } catch (e) {
      print('Database não inicializado: $e');
      return false;
    }
  }
  ...
  Future<void> resetDatabase() async {
    ...
    try {
      ...
      _database = null;
      // O banco será recriado na próxima chamada ao getter database
    } catch (e) {
      print('Erro ao resetar database: $e');
    }
  }
}
